...
use hpotter::docker::{
    ...
    ensure_db_volume, ensure_image, get_container_id, get_container_ip, get_container_logs,
    get_network_id, get_network_names, image_is_available, pull_image, start_container,
};
use std::sync::Arc;
use test_helpers::{EXPECTED_CONTAINER_LOGS, TEST_BAD_CONFIG, TEST_GOOD_CONFIG};
...
mod tests {
    ...
    async fn setup() -> () {
        ...
        let result = ensure_image(&docker_client, "hello-world:latest").await;
        assert!(result.is_ok());
    }
